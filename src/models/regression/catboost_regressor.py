"""
Module: catboost_regressor.py

Description:
    Train and evaluate a CatBoost Regressor baseline for
...
MODEL_ARTIFACT_PATH = (
    ARTIFACTS_DIR
    / "catboost_regressor_model.pkl"
)

PREPROCESSOR_ARTIFACT_PATH = (
    ARTIFACTS_DIR
    / "catboost_regressor_preprocessor.pkl"
)
...
def evaluate_model(
    ...
    """
    Evaluate the CatBoost regression model.
...
def train_catboost() -> dict[str, float]:
    ...
    """
    logger.info(
        "Starting CatBoost Regression training."
    )
    ...
    logger.info(
        "CatBoost Regression training completed."
    )
    ...
    print(
        "CATBOOST REGRESSION BASELINE SUMMARY"
    )
    print(
        "=" * 60
    )
